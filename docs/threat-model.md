...
| **Unwanted calls to somebody vulnerable** | Calls are off until the holder turns them on; each device refuses to ring otherwise, and never for somebody removed; "Not now" and a missed call ring out after 45 seconds | Anybody the holder has let in can call once calls are on; there is no per-person block yet | App (0.3.5); not yet tried on machines |
| **A call overheard or recorded on the way** | Encrypted end to end by the browser's own WebRTC, straight between the two devices; Holochain carries only the introduction; nothing is stored | The address-check server (Nextcloud's) learns the caller's internet address | Built on a tested library; not independently reviewed |
...
| **A flaw in something Hearth is built on** | Versions pinned; dependencies checked against the public advisory database on every push (0.3.4) | The check reports; a person has to act. **First run, 23 September 2026:** the code in every circle is clean (two build-time helpers unmaintained, no known holes). The Holochain program the desktop app ships contains `rustls` 0.23.42, which has a medium-rated TLS flaw (RUSTSEC-2026-0285, fixed in 0.23.45); every Hearth release has it, 0.2.4 included, and only a Holochain release can fix it — 0.7.1 is at release candidate. The test harness also pulls a high-rated `quinn-proto` flaw (RUSTSEC-2026-0185); whether the shipped program contains it could not be confirmed | CI |
